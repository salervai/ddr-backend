...
function registerMayaRewardSystem({ app, pool, botToken, authenticateRequest }) {
  ...
  async function ensureRewardDatabase() {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS maya_unlock_ad_sessions (
        id BIGSERIAL PRIMARY KEY,
        unlock_session_id BIGINT NOT NULL REFERENCES maya_unlock_sessions(id) ON DELETE CASCADE,
    ...
    await pool.query(`
      CREATE INDEX IF NOT EXISTS maya_unlock_ad_session_idx
      ON maya_unlock_ad_sessions(unlock_session_id, created_at)
    `);
  ...
  async function deliverVideo(session) {
    ...
    const result = await telegram('sendVideo', {
      ...
      video: videoInput,
      caption: `馃幀 ${video.title}\n\n鈴憋笍 This video will be deleted automatically after ${video.delivery_ttl_seconds} seconds.`,
  ...
  // Create one unique Monetag ymid for each rewarded ad. This prevents a single
  // frontend ymid from being reused for multiple paid events and makes postback
  // idempotency reliable.
  app.post('/api/reward/video/ad-session', authenticateRequest, async (req,res) => {
    try {
      const sessionToken = String(req.body?.session_token || '').trim();
      if (!sessionToken) return res.status(400).json({ok:false,error:'session_token is required'});
      const session = (await pool.query(`
        SELECT * FROM maya_unlock_sessions
        WHERE session_token=$1 AND user_id=$2 AND status='watching' AND expires_at>NOW()
        LIMIT 1`,[sessionToken,req.user.id])).rows[0];
      if (!session) return res.status(404).json({ok:false,error:'Unlock session not found or expired'});
      const ymid = `maya-unlock-ad-${session.id}-${Date.now()}-${randomToken(8)}`;
      const r = await pool.query(`
        INSERT INTO maya_unlock_ad_sessions(unlock_session_id,ymid)
        VALUES($1,$2) RETURNING id,ymid,status,created_at`,[session.id,ymid]);
      res.status(201).json({ok:true,ad_session:r.rows[0]});
    } catch(e) {
      console.error('Maya unlock ad session error:',e);
      res.status(500).json({ok:false,error:'Could not create ad session'});
    }
  });
  ...
  // query parameter in Monetag's postback URL.
  app.get('/api/reward/ad/postback', async (req,res) => {
    try {
      ...
      const postbackSecret = String(req.query.secret || '');
      const expectedSecret = String(process.env.MONETAG_POSTBACK_SECRET || '');

      if (provider === 'monetag' && expectedSecret && postbackSecret !== expectedSecret) {
        return res.status(403).send('forbidden');
      }
      if (provider === 'monetag' && !expectedSecret) {
      ...
      if (unlockToken) {
        ...
        if (unlock && ymid) {
          unlockAd = (await pool.query(`SELECT * FROM maya_unlock_ad_sessions WHERE unlock_session_id=$1 AND ymid=$2 AND status='watching' LIMIT 1`,[unlock.id,ymid])).rows[0] || null;
        }
      }
      if (!unlock && ymid) {
        const matched = await pool.query(`
          SELECT s.*, a.id AS ad_session_id, a.status AS ad_status
          FROM maya_unlock_ad_sessions a
          JOIN maya_unlock_sessions s ON s.id=a.unlock_session_id
          WHERE a.ymid=$1 AND s.status='watching'
          LIMIT 1`,[ymid]);
        if (matched.rows[0]) {
          unlock = matched.rows[0];
          unlockAd = matched.rows[0];
        } else {
          unlock = (await pool.query(`SELECT * FROM maya_unlock_sessions WHERE ymid=$1 AND status='watching' LIMIT 1`,[ymid])).rows[0] || null;
        }
      }
        ...
        try {
          if (unlockAd) {
            const claim = await pool.query(`
              UPDATE maya_unlock_ad_sessions
              SET status='verified',verified_at=NOW()
              WHERE id=$1 AND status='watching'
              RETURNING id`,[unlockAd.ad_session_id || unlockAd.id]);
            if (!claim.rows[0]) {
          ...
          const eventInsert = await pool.query(`
            INSERT INTO maya_ad_events(provider,event_id,ymid,user_id,unlock_session_id,event_type)
            VALUES($1,$2,$3,$4,$5,$6)
            ON CONFLICT(provider,event_id) DO NOTHING
            RETURNING id`,[provider,eventId,ymid,unlock.user_id,unlock.id,eventType]);
